...
let step=Number(player.mission1Step||0), choiceDone=false;
const steps=[
...
function save(){CodeRealm.save(player)}
function hud(){
 CodeRealm.normalize(player);
 const n=player.name||"Aventurero", info=CodeRealm.getLevelInfo(player.exp);
 $("#playerName").textContent=n.toUpperCase();$("#playerRank").textContent=info.rank;$("#playerLevel").textContent=info.level;
 $("#playerXp").textContent=player.exp;$("#playerXpNext").textContent=info.nextExp;$("#expResource").textContent=player.exp;
 $("#crystalsResource").textContent=player.crystals;$("#goldResource").textContent=player.gold;$("#avatarMini").textContent=n[0].toUpperCase();$("#playerXpBar").style.width=info.progress+"%";
}
function render(){
 ...
 if(step===4) area.innerHTML=`<div class="reward-box"><div class="star">✦</div><h2>PRIMER PASO COMPLETADO</h2><strong>+50 EXP</strong><p>Has comprendido la filosofía de aprendizaje de CODE REALM. La próxima misión será preparar Visual Studio Code y crear tu primera zona de trabajo.</p></div>`;
...
function next(){
 ...
 else if(step<steps.length-1){step++;player.mission1Step=step;save();render()}
 else complete();
}
function complete(){
 if(!player.mission1Completed){
   player.mission1Completed=true;
   player.mission1Step=steps.length-1;
   const result=CodeRealm.awardExp(player,50);
   hud();
   toast(`+50 EXP · Misión 1 completada${result.leveledUp?` · NIVEL ${result.newLevel}`:""}`);
 }
 setTimeout(()=>location.href="act1-map.html",900);
}
function toast(t){const el=$("#toast");el.textContent=t;el.classList.add("show");setTimeout(()=>el.classList.remove("show"),2200)}
$("#nextBtn").onclick=next;$("#prevBtn").onclick=()=>{if(step>0){step--;player.mission1Step=step;save();if(step<3)choiceDone=false;render()}};
$("#backHome").onclick=()=>location.href="home.html";
document.querySelectorAll(".step").forEach(b=>b.onclick=()=>{const target=Number(b.dataset.step);if(target<=step){step=target;player.mission1Step=step;save();render()}});
